...
If you're interested in attaining a board, checkout the [product page for the Modos Paper Dev Kit on Crowd Supply](https://www.crowdsupply.com/modos-tech/modos-paper-monitor). For complete monitor ready to be used out-of-the-box, checkout the [product page for the Modos Flow on Crowd Supply](https://www.crowdsupply.com/modos-tech/modos-flow).
...
If you already have a Glider board/ kit: For board setup, flashing, development build loops, and display configuration, start with [USAGE.md](USAGE.md).
...
- Specialized controller chip
    - Closed-source
        ...
        - T1000: Also known as IT8957, upgraded model of IT8951. It supports even higher resolution. It features a higher speed MIPI DSI interface to mitigate the slow speed of IT8951. Features an integrated OpenRISC based processor.
        - T2000: Upgraded model of T1000.
        - T2001: Also known as IT8959. Features an integrated RISC-V based processor. Works with large EPDs up to 2560x1920. Supports SPI, RGB, MIPI DSI interface.
...
- 00: Common voltage (typically around -1.5V)
- 01: Negative voltage (typically -15V)
- 10: Positive voltage (typically 15V)
- 11: No voltage (Hi-Z)
...
Some higher resolution panels (such as 25.3" ones, and 11.8" Gallery 3 panel) uses LVDS signaling instead of LVCMOS. The interface/ timing is mostly the same, except the SDCE0 is no longer used. Instead, a DRST sequence is performed on data lines to signal the start of line data transmission.

##### Multi-Voltage Panels

Some newer panels supports more than one high voltage input for each polarity. On normal panels, the only high voltage that can be applied to the panel is +/-15V. On multi-voltage panels, it's also possible to apply 2 more sets of middle voltages. For example, +/-6V, +/-10V, in addition to the normal +/-15V. Thus each pixel is now encoded in 3-bits:

- 000: Common voltage (VCOM)
- 001: Most positive voltage (VP3)
- 010: Secondary positive voltage (VP2)
- 011: Least positive voltage (VP1)
- 100: Least negative voltage (VN1)
- 101: Secondary negative voltage (VN2)
- 110: Most negative voltage (VN3)
- 111: No voltage (Hi-Z)

Note in some datasheets, naming of 1 and 2 are swapped: VP2/VN2 rather than VP1/VP2 is the least positive voltage. Always read the datasheet carefully to avoid supplying the wrong voltage to the panel.

Some MV panels exposes a 24-bit databus, with a 8 pixels x 3 bits/pixel layout. Some other MV panels exposes a 16-bit databus, with 4 bits/pixel but MSB unused (effectively a 12-bit bus).

Gallery and Spectra color screens often supports multi-voltage, but there are also monochrome panels supports multi-voltage. Greyscale renderition can take advantage of the additional voltages available.

##### AC VCOM Panels

Typically, VCOM voltage is supplied by a DC voltage source, with a constant voltage throughout the normal operation. However, some panels uses AC VCOM, meaning the VCOM is also altered during the driving process. This allows the screen to achive much higher pixel voltages with fixed voltage ratings. For example, when VCOM is near 0V, the maximum positive driving voltage is around 15V. However, if the VCOM is set to -15V, the maximum voltage across the pixel becomes the difference between -15V and 15V, which is 30V. However, when the VCOM is at -15V, it's no longer possible to create a negative voltage across the pixel, as the difference between -15V (VCOM) and -15V (driving voltage) is 0V. As a result, the VCOM is altered between -15V and 15V, so both -30V and 30V can be applied to the panel, just not at the same time.

##### Oxide-TFT Panels

Oxide-TFT panels uses the same interface/ timing as non-Oxide-TFT multi-voltage panels. The voltage is typically much higher, however the specific voltage depends on the screen used.

Typical voltage used:

| Rail | Non-Oxide-TFT | Oxide-TFT |
|-|-|-|
| VPOS | 15V | 24V |
| VNEG | -15V | -24V |
| VGH | 27V | 35V |
...
- VERSION: the version of the descriptor (should be 2.0 or 2.1)
...
- UNIDIR: if or not the waveform ignores the source greyscale level (only in version 2.1+)
- OUTBPP: bits per pixel used by the screen panel. 2 or 4, 4 for multi-voltage panels (only in version 2.1+)
...
#### Translating Waveform State Encodings

In general, for 2-bpp (non-multi-voltage) panels, the values stored in the wbf file is the same value used by the screen (the exact same bit value being sent on the data bus). However for multi-voltage screens, Eink somehow decides to store values that's different from the screen's encoding. For instance, on Gallery 3 panels, when the WBF stores the value 14, the actual value needs to be transmitted is 2. Spectra 6 waveforms use another set of codings. A translate tool is provided to decode them into raw values with specified coding.

- To decode waveform for Gallery 3 and (maybe) other oxide TFT panels: ```python3 translate.py -p igzo input.iwf```
- To decode waveform for Spectra 6 panels: ```python3 translate.py -p e6 input.iwf```
- To use non-multi-voltage waveforms on Gallery3/ oxide TFT multi-voltage panels: ```python3 translate.py -p pmv input.iwf```
...
#### Support For Longer Waveform

Hardware EPD controllers typically have a rather limited/ strict limitation of the waveform length. However, some screens such as Gallery Plus and Spectra 6 use waveform that can be several minutes long (thousands of frames). To allow storing such long waveforms, the waveform no longer cares about the source greyscale/ color, instead only storing one waveform for each destination greyscale. For example, for a 5-bpp waveform (32 colors), originally it needs to store 32 x 32 = 1024 different sequences, now it only needs to store 32 different sequences. So it could fit 32X long sequence with the same storage capacity. Some Eink supplied waveforms are stored in this way.
...
| V570         | ?        | VD1400/ VD1405 | Carta 1300              | 2023             |
| ?            | M        | ?              | Spectra (BWR)           | ?                |
...
- From the information available, it really looks like Carta 1000 and 1100 use the same film. If this is true, it may sound like a scam if they are named differently but use the same film. But this is actually okay. Just like how chips usually have a binning process, the same chip would sell under different product names with different performance levels. It's reasonable for Eink to do incremental improvements on the manufacturing process to yield better screens with the same film and call it under a different name, or simply binning screens to different grades and selling under different names.
- Carta 1200 and 1300 might also use the same film. Carta 1300 might have been used to desginate panels with oxide TFT backplane.
...
Since ~2023, Eink has started using Oxide-TFT backplane on selected new screen panels. Oxide-TFT allows higher driving voltage to be used for faster pixel response time. While not necessary, Oxide-TFT panels often features multi-voltage driver IC as well.
...
- If this is a fresh install, transfer over the generated Quicksand font files (`fonts/font_quicksand_16.bin`, `fonts/font_quicksand_20.bin`, and `fonts/font_quicksand_28.bin`) using the same method as the bitstream
...
| ED100UC1   | VB3300-KOA   | EUT        | 320          | 1600x1200   | Carta                       | 45%   | 16:1   | 2020  | TTL       | 40        | 40P-B   | Yes     |
| ES103TC1   | VB3300-KCA   | EKQ        | 320          | 1872x1404   | Carta 1.2                   | 40%   | 12:1   | 2016  | TTL       | 40        | 40P-B   |         |
| ED103TC2   | VB3300-KCD   |            | 320          | 1872x1404   | Carta                       | 43%   | 14:1   | 2019  | TTL       | 40        | 40P-B   |         |
...
| ED420TT1   | VA3200-RBA   |            | V220         | 2880x2160   | Pearl                       |       |        |       | TTL       | 50x2      |         |         |
