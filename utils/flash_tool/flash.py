...
def send_buffer(h, bin):
    if len(bin) % PKT_DATA_SIZE != 0:
        bin += bytearray(PKT_DATA_SIZE - len(bin) % PKT_DATA_SIZE)
    pkts = len(bin) // PKT_DATA_SIZE
    ...
    print(f'\nDone ({totaltime} seconds, {len(bin)/totaltime/1000:.1f}KB/s)')

def send_file(h, fn, target_fn):
    fp = open(fn, 'rb')
    bin = fp.read()
    fp.close()
    fsize = len(bin)
    send_cmd(h, USBCMD_RECV, 1, fsize % 65536, fsize // 65536, 0, 0, 0)
